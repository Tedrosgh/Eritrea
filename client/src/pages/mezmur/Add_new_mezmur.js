...
import { createPostsmezmur, getPostsmezmur } from "../../actions/postsActions";
import { createPost, createPostmezmur, updatePost } from "../../api/api";
...
const Add_new_mezmur = ({ currentId, setCurrentId }) => {
  ...
  const mezmurs = useSelector((state) =>
  ...
  const handleSubmit = evt => {
    ...
    if (currentId) {
        dispatch(
          getPostsmezmur(currentId, { ...startState, name: user?.result?.name })
        );
      } else {
        dispatch(createPostsmezmur({ ...startState, name: user?.result?.name }));
      }
      clear();
    };
...
//   };
  ...
  return (
    <div className="sunday">
      ...
      <form className="sunday" onSubmit={createmezmur} style={{display: "flex", flexDirection: "column"}} >
